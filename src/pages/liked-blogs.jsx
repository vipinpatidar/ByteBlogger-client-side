...
const LikedBlogs = () => {
  ...
  const {
    ...
    queryKey: ["likedBlogs", query],
    queryFn: async ({ pageParam }) => {
      const res = await makeRequest.get(
        `/favorites/liked-blogs?search=${query}&page=${pageParam}`
      );
      return res.data;
    },
    initialPageParam: 0,
  ...
  return (
    <div className=" mx-auto pb-12">
      ...
      <>
        {!isFetching && blogsData?.pages[0]?.likedBlogs?.length === 0 ? (
          <NoDataMessage message={"No blog found :)"} error={error} />
        ) : !error && blogsData?.pages[0]?.likedBlogs?.length > 0 ? (
          blogsData.pages?.map((page, pageIndex) => (
            <React.Fragment key={pageIndex}>
              {page?.likedBlogs?.map((blog, idx) => {
                // console.log(blog);
